...
/**
 * The chroma between them is lifted, which is a deliberate step outside the palette — a ramp
 * built only from the brand navy reads as smoke on a white page. These two are the anchors
 * that keep it recognisably the brand, so they are hit exactly.
 */
...
test('sampling is clamped, so a height outside the figure cannot run off either end', () => {
  const stops = ramp();
