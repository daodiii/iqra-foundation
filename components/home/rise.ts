 ...
 * as parameters, for the foot's table (Contact.tsx). A tall element is judged by its first
 * four fifths of a screen, so a phone's plate is open while its top is still in view.
 * `data-rise` marks the element once the script runs and motion is wanted; without either
 * the stylesheet's default (`--o: 1`) holds and everything simply stands. `onFrame` is
 * told every frame, for whatever a plate does with the number.
 */
